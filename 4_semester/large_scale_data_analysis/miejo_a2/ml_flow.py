...
def main():
    m, s, n = tracking_experiment(return_best=True)
    print("logging best model")
    with mlflow.start_run(run_name=f'logging best model'):
        mlflow.sklearn.log_model(m, 'models')
        mlflow.sklearn.save_model(m, 'models')
